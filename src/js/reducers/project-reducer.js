...
export default function (state, action) {
  switch (action.type) {
    ...
    case GO_BACK_TO_CATALOG_PAGE:
      const pageIndex = state.catalog.path.findIndex(page => page === action.newPage);
      return state.setIn(['catalog', 'page'], action.newPage)
...
function remove(state) {
  ...
  scene = scene.updateIn(['layers', scene.selectedLayer], layer => layer.withMutations((layer) => {
    const { lines: selectedLines, holes: selectedHoles, items: selectedItems } = layer.selected;
    unselectAllOp(layer);
...
export function rollback(state) {
  const { sceneHistory } = state;

  if (sceneHistory.isEmpty()) return state;

  const scene = sceneHistory
    .last()
...
function setProjectProperties(state, properties) {
  const scene = state.scene.merge(properties);
  return state.merge({
    mode: MODE_IDLE,
    scene,
    sceneHistory: state.sceneHistory.push(scene)
