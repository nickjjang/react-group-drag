...
function addLayer(state, name, altitude) {
  ...
  const layer = new Layer({ id: layerID, name, altitude });
  let { scene } = state;
  scene = scene.merge({
...
function setLayerProperties(state, layerID, properties) {
  ...
  layers = layers.sort((layerA, layerB) => (layerA.altitude !== layerB.altitude ? layerB.altitude - layerA.altitude : layerA.order - layerB.order));
...
function removeLayer(state, layerID) {
  let { scene } = state;
  const layers = scene.layers.delete(layerID);
