...
export function replaceLineVertex(layer, lineID, vertexIndex, x, y) {
  ...
  layer = layer.withMutations(layer => layer.withMutations((layer) => {
    const vertexID = line.vertices.get(vertexIndex);
...
export function splitLine(layer, lineID, x, y, catalog) {
  ...
  layer = layer.withMutations((layer) => {
    const line = layer.getIn(['lines', lineID]);
    const v0 = layer.vertices.get(line.vertices.get(0));
    const v1 = layer.vertices.get(line.vertices.get(1));
    const { x: x0, y: y0 } = v0;
    const { x: x1, y: y1 } = v1;
    ...
    const minVertex = Geometry.minVertex(v0, v1);
...
export function addLinesFromPoints(layer, type, points, catalog, properties, holes) {
  ...
  const pointsPair = points.zip(points.skip(1))
    .filterNot(([{ x: x1, y: y1 }, { x: x2, y: y2 }]) => x1 === x2 && y1 === y2);

  const lines = (new List()).withMutations((lines) => {
    layer = layer.withMutations((layer) => {
      pointsPair.forEach(([{ x: x1, y: y1 }, { x: x2, y: y2 }]) => {
        const { line } = addLine(layer, type, x1, y1, x2, y2, catalog, properties);
        if (holes) {
          holes.forEach((holeWithOffsetPoint) => {
            ...
            if (Geometry.isPointOnLineSegment(x1, y1, x2, y2, xp, yp)) {
              const newOffset = Geometry.pointPositionOnLineSegment(x1, y1, x2, y2, xp, yp);
...
export function addLineAvoidingIntersections(layer, type, x0, y0, x1, y1, catalog, oldProperties, oldHoles) {
  ...
  layer = layer.withMutations((layer) => {
    ...
    lines.forEach((line) => {
      const [v0, v1] = line.vertices.map(vertexID => vertices.get(vertexID)).toArray();

      const hasCommonEndpoint = (Geometry.samePoints(v0, points[0])
          || Geometry.samePoints(v0, points[1])
          || Geometry.samePoints(v1, points[0])
          || Geometry.samePoints(v1, points[1]));


      const intersection = Geometry.intersectionFromTwoLineSegment(
      ...
      if (intersection.type === 'colinear') {
        ...
        const orderedVertices = Geometry.orderVertices(points);

        layer.lines.get(line.id).holes.forEach((holeID) => {
          const hole = layer.holes.get(holeID);
          const oldLineLength = Geometry.pointsDistance(v0.x, v0.y, v1.x, v1.y);
          ...
          const xp = oldLineLength * offset * Math.cos(alpha) + v0.x;
          const yp = oldLineLength * offset * Math.sin(alpha) + v0.y;
...
export function removeVertex(layer, vertexID, relatedPrototype, relatedID) {
  ...
  layer = vertex.areas.size || vertex.lines.size
...
export function unselect(layer, prototype, ID) {
  return layer.withMutations((layer) => {
    ...
    const selected = ids.some(key => key === ID);
...
function opSetLinesAttributes(layer, prototype, ID, linesAttributes, catalog) {
  const lAttr = linesAttributes.toJS();
...
function opSetHolesAttributes(layer, prototype, ID, holesAttributes) {
  const hAttr = holesAttributes.toJS();
  const { offsetA, offsetB, offset } = hAttr;

  delete hAttr.offsetA;
  delete hAttr.offsetB;
  delete hAttr.offset;

  const misc = new Map({ _unitA: offsetA._unit, _unitB: offsetB._unit });
...
export function addArea(layer, type, verticesCoords, catalog) {
  ...
  layer = layer.withMutations((layer) => {
    ...
    const vertices = verticesCoords.map(v => addVertex(layer, v.x, v.y, 'areas', areaID).vertex.id);
...
export function removeArea(layer, areaID) {
  const area = layer.getIn(['areas', areaID]);
...
export function detectAndUpdateAreas(layer, catalog) {
  const verticesArray = []; // array with vertices coords
  let linesArray; // array with edges

  const vertexID_to_verticesArrayIndex = {};
  const verticesArrayIndex_to_vertexID = {};
  ...
  const innerCyclesByVerticesArrayIndex = calculateInnerCyles(verticesArray, linesArray);
  ...
  innerCyclesByVerticesID = innerCyclesByVerticesID.map(area => (isClockWiseOrder(area.map(vertexID => layer.vertices.get(vertexID))) ? area.reverse() : area));
  ...
  layer = layer.withMutations((layer) => {
    ...
    innerCyclesByVerticesID.forEach((cycle, ind) => {
      const areaInUse = layer.areas.find(area => sameSet(area.vertices, cycle));

      if (areaInUse) {
        areaIDs[ind] = areaInUse.id;
        layer.setIn(['areas', areaIDs[ind], 'holes'], new List());
      } else {
        const areaVerticesCoords = cycle.map(vertexId => layer.vertices.get(vertexId));
        const { area } = addArea(layer, 'area', areaVerticesCoords, catalog);
        areaIDs[ind] = area.id;
      }
    });
    ...
    const verticesCoordsForArea = areaIDs.map((id) => {
      const vertices = layer.areas.get(id).vertices.map(vertexID => layer.vertices.get(vertexID));
      return { id, vertices };
    });
    ...
    for (i = 0; i < verticesCoordsForArea.length; i++) {
      ...
      const areaVerticesList = verticesCoordsForArea[i].vertices.flatten().toArray();
      for (j = 0; j < verticesCoordsForArea.length; j++) {
        if (i !== j) {
          const isHole = ContainsPoint(areaVerticesList,
    ...
    areaIDs.forEach((areaID) => {
      const doubleHoles = new Set();
      let areaHoles = layer.getIn(['areas', areaID, 'holes']);
      areaHoles.forEach((areaHoleID) => {
        const holesOfholes = layer.getIn(['areas', areaHoleID, 'holes']);
        holesOfholes.forEach((holeID) => {
          const holeIndex = areaHoles.indexOf(holeID);
          if (holeIndex !== -1) {
      ...
      doubleHoles.forEach((doubleHoleID) => {
        const holeIndex = areaHoles.indexOf(doubleHoleID);
...
export function removeHole(layer, holeID) {
  ...
  layer = layer.withMutations((layer) => {
    ...
    layer.updateIn(['lines', hole.line, 'holes'], (holes) => {
      const index = holes.findIndex(ID => holeID === ID);
