...
import {
  ...
  MODE_DRAGGING_VERTEX, MODE_DRAGGING_ITEM, MODE_RESIZING_ITEM, MODE_DRAGGING_HOLE, MODE_FITTING_IMAGE, MODE_UPLOADING_IMAGE,
  MODE_ROTATING_ITEM
} from '../../constants';
...
const searchInputStyle = {
  fontSize: '1em', width: '100%', height: '1em', padding: '1em 0.5em'
};
...
export default class PanelLayerElement extends Component {
  constructor(props, context) {
    ...
    const layer = props.layers.get(props.selectedLayer);
    const elements = {
      lines: layer.lines || {},
      holes: layer.holes || {},
      items: layer.items || {}
  ...
  componentWillReceiveProps(nextProps) {
    const layer = nextProps.layers.get(nextProps.selectedLayer);

    if (diff(this.props.layers, nextProps.layers).size === 0) return;

    const elements = {
      lines: layer ? layer.lines : {},
      holes: layer ? layer.holes : {},
      items: layer ? layer.items : {}
    };
  ...
  render() {
    ...
    const layer = this.props.layers.get(this.props.selectedLayer);

    return (
      <Panel name={this.context.translator.t('Elements on layer {0}', layer ? layer.name : '')}>
